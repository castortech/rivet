...
export async function saveDatasetsFile(projectFilePath: string, project: Project) {
  const serializedDatasets = await getSerializedDatasets(projectFilePath, project);
	if (serializedDatasets) {
...
export async function getSerializedDatasets(projectFilePath: string, project: Project) {
  ...
  if (datasets.length > 0 || (await exists(dataPath))) {
    return serializeDatasets(datasets);
  }
}
