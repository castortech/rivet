...
| Prompts | `object`         | MCP prompts returned by the MCP server | Returned from the `prompts/list` method. Only availabe when Output Prompts is enabled. |
...
For HTTP mode, Use custom headers similar to what you would do in a chat node.
